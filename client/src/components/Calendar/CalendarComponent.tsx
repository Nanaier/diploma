...
interface CalendarComponentProps {
  user: User | null;
  events: Event[];
  assignedUsers: User[]; // For psychologists
  availableExercises: any[]; // Exercise options
  onUpdateEvent: (updatedEvent: any) => Promise<void>;
  onCreateEvent: (newEvent: Event) => Promise<Event>;
...
export default function CalendarComponent({
  ...
  const handleUpdateComment = async (updatedEvent: any) => {
    try {
      setEvents((prev) =>
        prev.map((event) =>
          event.id === selectedEvent?.id
  ...
  const handleCreateEvent = async () => {
    try {
      ...
      const eventToCreate = {
        ...newEvent,
        userId:
          isPsychologist && newEvent.userId != 0 && newEvent.userId
            ? newEvent.userId
            : user?.id,
      } as Event;

      const createdEvent = await onCreateEvent(eventToCreate);
      console.log(isPsychologist);
      console.log(isPsychologist && newEvent.userId != 0 && newEvent.userId);
  ...
  if (isLoading) {
    return (
      <div className="h-screen p-4 bg-gray-50 flex items-center justify-center">
        <LoadingSpinner />
      </div>
    );
  }
    ...
    <div className="h-screen p-4 bg-gray-50">
      <div className="bg-white rounded-lg shadow-sm p-4 h-full relative">
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 flex justify-end space-x-3">
                    <button
                      onClick={closeModal}
                      className="px-4 py-2 text-sm font-medium rounded-lg cursor-pointer border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors"
                    >
                      {t("Close")}
                    </button>
                    ...
                    {user?.id === selectedEvent.user.id &&
                      ...
                      new Date(selectedEvent.start) > new Date() && (
                        <button
                          ...
                          className="px-4 py-2 text-sm rounded bg-blue-500 text-white cursor-pointer hover:bg-blue-600 transition-colors"
                        >
                          {t("Edit")}
                        </button>
                      )}
        ...
        {isAddCommentModalOpen && (
          <AddMeetingCommentModal
            comment={selectedEvent.SessionComment[0]?.comment || ""}
            onSave={(newComment) =>
              handleUpdateComment({ ...selectedEvent, comment: newComment })
            }
